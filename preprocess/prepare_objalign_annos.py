...
parser.add_argument('--train_iou_thres', type=float, default=0.5)
parser.add_argument('--overlap_iou_thres', type=float, default=0.9)
...
for split in ["train", "val"]:
    ...
    if segmentor == 'deva':
        ...
        instance_attribute_file = f"annotations/scannet_{segmentor}_{split}_attributes{version}_{args.overlap_iou_thres}.pt"
        instance_attrs = torch.load(instance_attribute_file, map_location='cpu')
    scannet_attribute_file = f"annotations/scannet_{split}_attributes.pt"
    scannet_attrs = torch.load(scannet_attribute_file, map_location='cpu')
    scene_ids = [scene_id for scene_id in scannet_attrs.keys()]
    print(scene_ids)
        ...
        for obj_id in range(gt_num):
            ...
            if segmentor == 'deva':
                ...
                for pred_id in range(instance_locs.shape[0]):
                    if "valid" in instance_attrs[scene_id]:
                        valid_mask = instance_attrs[scene_id]["valid"]
                        if valid_mask[pred_id] == 0:
                            continue
    ...
    print(len(new_annos))
    with open(f"annotations/obj_align_{segmentor}_{split}{version}_{args.overlap_iou_thres}_trainiou_{args.train_iou_thres}.json", 'w') as f:
