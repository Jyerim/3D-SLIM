...
default_instance_attr_file = "annotations/scannet_mask3d_val_attributes_nonoverlap_0.9.pt"
...
def calc_scan2cap_score(preds, tokenizer, scorers, config=None):
    ...
    for pred in preds:
        ...
        if iou >= 0.25:
            tmp_preds_iou25[key] = [{'caption': f"sos {pred['pred']} eos".lower().replace('\n', ' ')}]
        else:
            tmp_preds_iou25[key] = [{'caption': f"sos eos"}]
        if iou >= 0.5:
            tmp_preds_iou50[key] = [{'caption': f"sos {pred['pred']} eos".lower().replace('\n', ' ')}]
        else:
...
def calc_multi3dref_location_score(preds, config=None):
    ...
    for i, pred in enumerate(preds):
        ...
        if eval_type in ['zt_wo_d', 'zt_w_d']:
            ...
            if len(pred_locs_list) == 0 and len(gt_ids) == 0:
                iou_25_f1 = iou_50_f1 = 1
            elif len(pred_locs_list) == 0 or len(gt_ids) == 0:
                iou_25_f1 = iou_50_f1 = 0
            else:
                pred_corners_list = []
                gt_corners_list = []
