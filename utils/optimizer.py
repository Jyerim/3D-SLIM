...
def add_different_lr(named_param_tuples_or_model, diff_lr_names, diff_lrs, diff_wds, default_lr):
    ...
    for name, p, wd in named_param_tuples_or_model:
        use_diff_lr = False
        for diff_name, diff_lr, diff_wd in zip(diff_lr_names, diff_lrs, diff_wds):
...
def create_optimizer(args, model, global_config, filter_bias_and_bn=True):
    ...
    lr_multi = global_config.batch_size * global_config.gpu_num * args.accum_iter
