...
def get_media_types_gradnorm(loaders):
    """Extract task names from DataLoaders, falling back to dataset media_type if not set."""
    media_types = []
    for loader in loaders:
        if hasattr(loader, 'task_name'):
            media_types.append(loader.task_name)
        else:
            dataset = loader.dataset
            if isinstance(dataset, ConcatDataset) and hasattr(dataset, 'media_type'):
                media_types.append(dataset.media_type)
            else:
                media_types.append(dataset.media_type)
    return media_types
