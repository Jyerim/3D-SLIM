...
def train(
    ...
    loss_names = ["loss", "obj_norm", "obj_img_norm","objid_norm" ,"max_seq_len"]
    ...
    for i, (media_type, batch) in enumerate(iterator):
        ...
        for name in loss_names:
            ...
            value = value if isinstance(value, float) or isinstance(value, int) else value.item()
...
def evaluate(
    ...
    tmp_path = os.path.join(
        config.output_dir,
        f"preds_epoch{epoch}_step{global_step}_rank{get_rank()}_{eval_name}.jsonl"
    )
    ...
    dist.barrier()
    if is_main_process():
        # JSONL 파일들을 모아서 하나의 리스트로 불러온 뒤 정렬 & 최종 JSON으로 저장
        all_preds = []
        for rank in range(config.gpu_num):
            path = os.path.join(
                config.output_dir,
                f"preds_epoch{epoch}_step{global_step}_rank{rank}_{eval_name}.jsonl"
            )
            ...
            os.remove(path)
        all_preds.sort(key=lambda x: f"{x['scene_id']}_{x['gt_id']:03}_{x['qid']}")
        final_path = os.path.join(
            config.output_dir,
            f"preds_epoch{epoch}_step{global_step}_{eval_name}.json"
        )
        with open(final_path, 'w', encoding='utf-8') as fw:
            json.dump(all_preds, fw, indent=4, ensure_ascii=False)
        save_preds = all_preds

    val_scores = {}
    if is_main_process() and save_preds is not None:
        if eval_name == 'scanqa':
        ...
        elif eval_name in ["sqa3d", "sqa3d_val", "sqa3d_test"]:
