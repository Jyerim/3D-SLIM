...
class ValDataset(BaseDataset):
    ...
    def __init__(self, ann_list, dataset_name, config, **kwargs):
        ...
        if feat_file in ValDataset.cached_feats and img_feat_file in ValDataset.cached_feats:
            cached = ValDataset.cached_feats[feat_file]
            if len(cached) == 3:
                self.scene_feats, self.scene_masks, self.scene_locs = cached
            else:
                self.scene_feats, self.scene_masks = cached
            ...
            if self.attributes is None:
                ...
                self.scene_img_feats = self.scene_masks = self.scene_locs = None
            else:
                self.scene_feats, self.scene_img_feats, self.scene_masks, self.scene_locs = self.prepare_scene_features()
                           
            ValDataset.cached_feats[feat_file] = (self.scene_feats, self.scene_masks, self.scene_locs)
    ...
    def __getitem__(self, index):
        scene_id, scene_feat, scene_img_feat, scene_mask, scene_locs, assigned_ids = self.get_anno(index, self.permute, 'val')
        ...
        if "ref_captions" in self.anno[index]:
            ref_captions = self.anno[index]["ref_captions"].copy()
        else:
            caption = self.anno[index].get("caption")
            ref_captions = caption.copy() if isinstance(caption, list) else []
